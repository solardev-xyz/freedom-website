...
<!-- DRAFT. Before publishing: delete this comment and put the date as
*D Month YYYY* on the line directly after the heading (the build reads it
from there); replace the [img] slots with <figure> blocks and set `image:`
in the front-matter; register the post in the POSTS array in
scripts/build.js. Facts checked against the release/0.8.5 changelog on
2026-09-10 (see freedom-org/research/crops-pitch/sources.md); re-check
"Under the hood" if further candidates land. -->
...
[img: the browser on an ordinary site, with the find bar and the download shelf visible]
...
- Remappable keyboard shortcuts, and page zoom (thanks @alexwbend).
...
- **Myotis.** An experimental peer-to-peer light client for Ethereum and Gnosis by @biafra23, off by default. Reads are proven against a chain head it syncs from peers.
...
[img: the shield popover over a web3:// app]
...
Not in this release: extensions and sync.
